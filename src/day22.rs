...
fn overlap(a: isize, b: isize, c: isize, d: isize) -> Option<(isize, isize)> {
    ...
    if a0 < b0 && a1 < b0 { return None; }
    if a0 > b1 && a1 > b1 { return None; }
...
fn parse(s: &str) -> (bool, Cube) {
    ...
    let mut x_range = cube_part.next().unwrap().split('=').skip(1).next().unwrap().split("..");
    let mut y_range = cube_part.next().unwrap().split('=').skip(1).next().unwrap().split("..");
    let mut z_range = cube_part.next().unwrap().split('=').skip(1).next().unwrap().split("..");
...
fn part1(ops: &[(bool, Cube)], init: &Cube) -> usize {
    ops.iter()
        .flat_map(|(flag, cube)|
            cube.and(&init).map(|c| c.dots()).unwrap_or_default()
                .into_iter()
                .map(|d| (flag, d))
                .collect::<Vec<_>>())
        ...
        for (x0, x1) in cut_one(this.x0, this.x1, that.x0, that.x1) {
            for (y0, y1) in cut_one(this.y0, this.y1, that.y0, that.y1) {
                for (z0, z1) in cut_one(this.z0, this.z1, that.z0, that.z1) {
                    let cube = Cube { x0, x1, y0, y1, z0, z1 };
                    if cube == that {
                        continue;
                    }
                    cubes.push(cube);
...
fn part2(ops: &[(bool, Cube)]) -> usize {
    ...
    for (flag, cube) in ops {
            ...
            .flat_map(|x| cut(&x, &cube))
            .collect();
        if *flag {
            done.push(cube.clone());
        }
    }
...
fn main() {
    ...
    let init = Cube { x0: -50, x1: 50, y0: -50, y1: 50, z0: -50, z1: 50 };
    let part1 = part1(&ops, &init);
    println!("{}", part1);

    let part1_ops = ops.iter()
        .cloned()
...
mod tests {
    ...
    #[test]
    fn test_parse() {
        assert_eq!(parse("on x=-20..26,y=-36..17,z=-47..7"), (true, (Cube { x0: -20, x1: 26, y0: -36, y1: 17, z0: -47, z1: 7 })));
    }
