...
fn align(this: &[V3], that: &[V3]) -> Option<(V3, HashSet<V3>)> {
    for a in this {
        for op1 in ops() {
            let this_set: HashSet<V3> = this.iter()
                .cloned()
                .map(|x| x.sub(a))
                .map(|x| op1.run(&x))
                .collect();
            ...
            for op2 in ops() {
                for b in that {
                    let that_set: HashSet<V3> = that.iter()
                        .cloned()
                        .map(|x| x.sub(b))
                        .map(|x| op2.run(&x))
                        .collect();
...
fn main() {
    let scanners: Vec<Vec<V3>> = lines()
        ...
        .map(|chunk| chunk.into_iter()
            .skip(1)
            .map(|line| parse_vec(line))
            .collect())
        .collect();

    let mut done: HashSet<V3> = scanners.get(0).unwrap()
        .iter()
        .cloned()
        .collect();
    let mut todo = scanners.into_iter()
        .skip(1)
        .collect::<Vec<_>>();
